...
class Pinata():
    ...
    def get_pins(self):
        response = requests.get('https://api.pinata.cloud/data/pinList?status=pinned', headers=self.headers).json()
        if "error" in response:
            return {'status': 'error', 'message':response['error']['details']}
        y = []
        for x in response["rows"]:
            x["URL"] = 'https://gateway.pinata.cloud/ipfs/'+ x["ipfs_pin_hash"]
            y.append(x)
        response["rows"] = y
        return {'status': 'success', 'data': response}
